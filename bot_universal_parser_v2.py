...
# ---------- Universal / Flexible parser ----------

QUESTION_RE = re.compile(r"(?im)^\s*(?:(?:Q(?=\s*(?:\d+|NO\.?|NUMBER|नं\.?|नंबर|[\.\)\:\-]))|QUESTION\b)(?:\s*(?:NO\.?|NUMBER|नं\.?|नंबर))?\s*(\d+)?\s*[\.\)\:\-]?\s*|प्रश्न(?:\s*(?:क्रमांक|नं\.?|नंबर))?\s*(\d+)?\s*[\.\)\:\-]?\s*|(\d+)\s*[\.\)\:\-]\s*)(.+?)\s*$")
OPTION_EN_RE = re.compile(r"(?im)^\s*(?:OPTION\s*)?([ABCDabcd])\s*[\.\)\:\-]\s*(.+?)\s*$")
OPTION_HI_RE = re.compile(r"(?im)^\s*(?:विकल्प\s*)?([कखगघ])\s*[\.\)\:\-]\s*(.+?)\s*$")
ANSWER_RE = re.compile(r"(?im)^\s*(?:ANSWER|ANS|CORRECT\s*ANSWER|RIGHT\s*ANSWER|CORRECT|RIGHT|उत्तर|सही\s*उत्तर|सही)\s*[.:=\-]?\s*(?:OPTION\s*)?([ABCDabcd1-4कखगघ])\b")
EXPLANATION_RE = re.compile(r"(?ims)^\s*(?:EXPLANATION|WHY|REASON|EXPLAIN|व्याख्या|कारण|विवरण|स्पष्टीकरण)\s*[:\-]\s*(.*?)(?=^\s*(?:ANSWER\s*KEY|ANSWER|ANS|CORRECT\s*ANSWER|RIGHT\s*ANSWER|उत्तर\s*कुंजी|उत्तर|सही\s*उत्तर|KEY)\s*[:=\-]?|^\s*(?:EXPLANATION|WHY|REASON|EXPLAIN|व्याख्या|कारण|विवरण|स्पष्टीकरण)\s*[:\-]|\Z)")

def clean_line(s):
    return re.sub(r"\s+", " ", s.strip())

def _answer_index(token):
    token = str(token).strip().upper()
    if token in "ABCD":
        return ord(token) - 65
    if token in {"1", "2", "3", "4"}:
        return int(token) - 1
    hi_map = {"क": 0, "ख": 1, "ग": 2, "घ": 3}
    return hi_map.get(token)
...
def extract_meta(text):
    ...
    topic = val(r"^\s*(?:TOPIC|CHAPTER|अध्याय|टॉपिक)\s*[:\-]\s*(.+)$")
    return title, exam, subject, topic

def extract_answer_key(text):
    answers = {}
    pats = [r"(?im)^\s*(?:ANSWER\s*KEY|ANSWERS?|उत्तर\s*कुंजी)\s*[:=\-]\s*(.+?)\s*$", r"(?im)^\s*KEY\s*[:=\-]\s*(.+?)\s*$"]
    for pat in pats:
        for m in re.finditer(pat, text):
            chunk = m.group(1)
            for q, a in re.findall(r"(?i)(?:Q\s*)?(\d+)\s*[\-:\.)]\s*([ABCDabcd1-4कखगघ])", chunk):
                idx = _answer_index(a)
                if idx is not None: answers[int(q)] = idx
            for q, a in re.findall(r"(?i)(\d+)\s*=\s*([ABCDabcd1-4कखगघ])", chunk):
                idx = _answer_index(a)
                if idx is not None: answers[int(q)] = idx
    return answers
...
def extract_inline_answer(block):
    ...
    return _answer_index(m.group(1)) if m else None

def extract_explanation(block):
    m = EXPLANATION_RE.search(block)
    return clean_line(m.group(1)) if m else ""

def _extract_options(block):
    options = {}
    for letter, value in OPTION_EN_RE.findall(block):
        options[letter.upper()] = clean_line(value)
    if set(options) == {"A", "B", "C", "D"}:
        return options
    hi_map = {"क":"A", "ख":"B", "ग":"C", "घ":"D"}
    for letter, value in OPTION_HI_RE.findall(block):
        options[hi_map[letter]] = clean_line(value)
    return options

def parse_quiz(text):
    text = text.replace("\r\n", "\n").replace("\r", "\n")
    # Remove common Markdown/list decoration so AI-generated Quiz text also parses.
    text = re.sub(r"(?m)^\s*(?:#{1,6}\s+|>\s+|[*_`]+\s*)", "", text)
    text = re.sub(r"[*_`]+", "", text)
    text = re.sub(r"(?im)^\s*Q\s*[\.)]\s*(\d+)", r"Q\1", text)
    text = text.strip()
    if not text: raise ValueError("Quiz text खाली है।")
    title, exam, subject, topic = extract_meta(text)
    key = extract_answer_key(text)
    matches = list(QUESTION_RE.finditer(text))
    if not matches:
        raise ValueError("सवाल नहीं मिले। Q1., Q1), 1., 1), प्रश्न 1: जैसे सामान्य formats रखें।")
    questions = []
    for idx, m in enumerate(matches):
        end = matches[idx + 1].start() if idx + 1 < len(matches) else len(text)
        block = text[m.end():end]
        q_no = int(m.group(1) or m.group(2) or m.group(3) or (idx + 1))
        qtext = clean_line(m.group(4))
        options = _extract_options(block)
        if set(options) != {"A", "B", "C", "D"}:
            found = ", ".join(sorted(options)) if options else "कोई नहीं"
            raise ValueError(f"प्रश्न {idx+1} में A, B, C, D चारों options नहीं मिले (मिले: {found})। A., A), A:, क), ख), ग), घ) आदि स्वीकार हैं।")
        answer = extract_inline_answer(block)
        if answer is None: answer = key.get(q_no)
        if answer is None:
            mnum = re.search(r"(?im)^\s*(?:ANSWER|ANS|CORRECT|RIGHT|उत्तर|सही)\s*[.:=\-]?\s*(?:OPTION\s*)?([1-4कखगघ])\b", block)
            if mnum: answer = int(mnum.group(1)) - 1
        if answer not in range(4):
            raise ValueError(f"प्रश्न {idx+1} का सही उत्तर नहीं मिला। ANSWER: B, उत्तर: B, Correct Answer: 2 या Answer Key: 1-B जैसे formats स्वीकार हैं।")
        questions.append({"q_no": len(questions)+1, "question": qtext, "options": [options["A"], options["B"], options["C"], options["D"]], "answer": answer, "explanation": extract_explanation(block)})
    if len(questions) > MAX_QUESTIONS: raise ValueError(f"एक Quiz में अधिकतम {MAX_QUESTIONS} सवाल रखें।")
    return {"title": title, "exam": exam, "subject": subject, "topic": topic, "questions": questions}
...
async def collect_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"✅ हिस्सा मिल गया ({len(total):,} characters)।\n\nसब भेजने के बाद नीचे **✅ Quiz Done** दबाएँ।", reply_markup=import_keyboard(), parse_mode="Markdown")
...
async def send_current(user_id, context_or_app, chat_id):
    ...
    keyboard = [
        [InlineKeyboardButton(
            f"{letter}. {text}",
            callback_data=f"ans:{q['id']}:{original}:{'ABCD'.index(letter)}",
        )]
...
async def answer(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not allowed(update):
        return
    _, qid_text, opt_text, correct_display_text = query.data.split(":")
    qid, selected, correct_display_index = int(qid_text), int(opt_text), int(correct_display_text)
    uid = update.effective_user.id
    with db() as conn:
        attempt = conn.execute("SELECT * FROM attempts WHERE user_id=%s", (uid,)).fetchone()
    if not attempt:
    ...
    correct_letter = "ABCD"[correct_display_index]
    text = "✅ सही!" if correct else f"❌ गलत!\n\nसही उत्तर: {correct_letter}"
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(answer, pattern=r"^ans:\d+:[0-3]:[0-3]$"))

    # Menu handlers must come before the generic text collector.
    app.add_handler(MessageHandler(filters.Regex(r"^(📝 Quiz|➕ Add Quiz|🔄 ReAttempt|📊 Stats|📚 Categories|⚙️ Settings|❓ Help)$"), text_menu))
    app.add_handler(MessageHandler(filters.Regex(r"^✅ Quiz Done$"), import_done))
    app.add_handler(MessageHandler(filters.Regex(r"^❌ Cancel Import$"), cancel))
