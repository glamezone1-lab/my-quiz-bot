...
async def start(u,c):await u.message.reply_text("👋 *My Quiz Bot*\n\nनीचे से विकल्प चुनें।",parse_mode="Markdown",reply_markup=home_k(u.effective_user.id))
...
async def title(u,c):admin[u.effective_user.id]["quiz"]["title"]=u.message.text.strip();await u.message.reply_text("Type: 1=MCQ, 2=TRUE_FALSE, 3=MULTI, 4=TEXT, 5=IMAGE_MCQ");return TYPE
async def typ(u,c):
    m={"1":"MCQ","2":"TRUE_FALSE","3":"MULTI","4":"TEXT","5":"IMAGE_MCQ"};v=u.message.text.strip().upper()
...
async def qtext(u,c):
    ...
    if x["qtype"]=="TEXT":await u.message.reply_text("सही text answer भेजें.");return ANSWER
...
async def more(u,c):
    ...
    if v in ("हाँ","हां","yes","y"):await u.message.reply_text("Next type: 1=MCQ, 2=TRUE_FALSE, 3=MULTI, 4=TEXT, 5=IMAGE_MCQ");return TYPE
...
def parse_import(s):
    ...
    for raw in s.splitlines():
        ...
        if cur is None:
            ...
            if cur["qtype"]=="TEXT":cur["answers"]=[v]
    ...
    for i,z in enumerate(qs,1):
        if z["qtype"] not in ("MCQ","TRUE_FALSE","MULTI","TEXT","IMAGE_MCQ"):raise ValueError(f"Question {i}: Type गलत.")
