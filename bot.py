...
KEEPALIVE = os.getenv("KEEPALIVE", "false").strip().lower() == "true"
...
async def revision_worker(app):
    """Send due revision notices and reschedule them for another 24 hours.

    The due time is stored in SQLite, so a process restart does not lose the
    schedule (subject to Render's filesystem persistence limits on Free).
    """
    while True:
        try:
            if ADMIN_ID:
...
async def keepalive_worker():
    """Optional self-ping while the Render instance is awake.

    This does not wake a sleeping Free instance by itself. For automatic
    wake-up after Render sleeps, use an external monitor to request /health.
    """
    if not KEEPALIVE or not RENDER_EXTERNAL_URL:
        return

    url = RENDER_EXTERNAL_URL + "/health"
    log.info("Keepalive enabled: %s", url)
    while True:
        try:
            req = urllib.request.Request(
                url, headers={"User-Agent": "QuizBot-Health/1.0"}
            )
...
def main():
    ...
    async def post_init(application):
        application.create_task(revision_worker(application), name="revision-worker")
        if KEEPALIVE and RENDER_EXTERNAL_URL:
            application.create_task(keepalive_worker(), name="keepalive-worker")
