...
QUESTION_RE = re.compile(r"(?im)^\s*(?:(?:Q|QUESTION|प्रश्न)\s*(\d+)\s*[\.\)\:\-]\s*|(\d+)\s*[\.\)\:\-]\s*)(.+?)\s*$")
OPTION_EN_RE = re.compile(r"(?im)^\s*([ABCDabcd])\s*[\.\)\:\-]\s*(.+?)\s*$")
OPTION_HI_RE = re.compile(r"(?im)^\s*([कखगघ])\s*[\.\)\:\-]\s*(.+?)\s*$")
ANSWER_RE = re.compile(r"(?im)^\s*(?:ANSWER|ANS|CORRECT\s*ANSWER|RIGHT\s*ANSWER|CORRECT|RIGHT|उत्तर|सही\s*उत्तर|सही)\s*[:=\-]?\s*(?:OPTION\s*)?([ABCDabcd1-4])\b")
EXPLANATION_RE = re.compile(r"(?ims)^\s*(?:EXPLANATION|WHY|REASON|व्याख्या|कारण|विवरण)\s*[:\-]\s*(.*?)(?=^\s*(?:ANSWER|ANS|CORRECT\s*ANSWER|RIGHT\s*ANSWER|उत्तर|सही\s*उत्तर)\s*[:=\-]?|^\s*(?:EXPLANATION|WHY|REASON|व्याख्या|कारण|विवरण)\s*[:\-]|\Z)")
...
def extract_answer_key(text):
    ...
    for pat in pats:
        for m in re.finditer(pat, text):
            chunk = m.group(1)
            for q, a in re.findall(r"(?i)(?:Q\s*)?(\d+)\s*[\-:\.)]\s*([ABCDabcd1-4])", chunk):
                idx = _answer_index(a)
                if idx is not None: answers[int(q)] = idx
            for q, a in re.findall(r"(?i)(\d+)\s*=\s*([ABCDabcd1-4])", chunk):
                idx = _answer_index(a)
                if idx is not None: answers[int(q)] = idx
    return answers
...
def parse_quiz(text):
    ...
    for idx, m in enumerate(matches):
        ...
        if answer is None:
            mnum = re.search(r"(?im)^\s*(?:ANSWER|ANS|CORRECT|RIGHT|उत्तर|सही)\s*[:=\-]?\s*([1-4])\b", block)
