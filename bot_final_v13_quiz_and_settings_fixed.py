...
async def collect_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Telegram clients can normalize emoji variation selectors differently,
    # so compare a normalized copy of the button text.
    ...
    if normalized == "📝 Quiz":
        ...
        return
    if normalized == "➕ Add Quiz":
        await import_start(update, context)
        return
    if normalized == "🔄 ReAttempt":
        context.user_data["importing"] = False
        context.user_data.pop("import_text", None)
    ...
    if normalized == "📚 Categories":
        ...
        return
    if normalized in ("⚙ Settings", "Settings", "⚙️ Settings"):
        context.user_data["importing"] = False
        context.user_data.pop("import_text", None)
...
async def start_quiz_for_user(user_id, chat_id, context, quiz_id):
    ...
    # Replace any old active session explicitly. This is more robust than
    # relying on an ON CONFLICT target because older databases may have an
    # attempts table created by a previous bot version.
    try:
        with db() as conn:
            conn.execute("DELETE FROM attempts WHERE user_id = %s", (user_id,))
            conn.execute(
                """
                INSERT INTO attempts
                (user_id, quiz_id, question_ids, position, score, mode, option_order)
                VALUES (%s, %s, %s::jsonb, 0, 0, 'quiz', %s::jsonb)
                """,
                (
                    user_id,
                    quiz_id,
            ...
            # Verify the row before leaving the transaction.
            check = conn.execute(
                "SELECT user_id, quiz_id, question_ids, position FROM attempts WHERE user_id = %s",
                (user_id,),
            ).fetchone()
            if not check or int(check["quiz_id"]) != int(quiz_id):
                raise RuntimeError("attempt row verification failed")
            conn.commit()
    except Exception as exc:
        log.exception("Attempt creation failed: user_id=%s quiz_id=%s", user_id, quiz_id)
        await context.bot.send_message(
            chat_id=chat_id,
            text=(
                "❌ Quiz session नहीं बन पाया।\n\n"
                "Database/attempt error मिला है।\n"
                "कृपया उसी Quiz को फिर से चुनें।"
            ),
            reply_markup=main_menu(),
        )
        return False
    ...
    try:
        ...
        log.exception("First question send failed: user_id=%s quiz_id=%s", user_id, quiz_id)
        # Do not leave a broken attempt behind.
        try:
            with db() as conn:
                conn.execute("DELETE FROM attempts WHERE user_id = %s", (user_id,))
                conn.commit()
        except Exception:
            log.exception("Failed to clean broken attempt for user_id=%s", user_id)
        await context.bot.send_message(
            chat_id=chat_id,
            text="❌ पहला सवाल भेजते समय समस्या हुई। Quiz को फिर से चुनें।",
            reply_markup=main_menu(),
        )
        return False
    return True
...
async def settings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        log.exception("Settings display failed for user_id=%s", update.effective_user.id)
        await update.effective_message.reply_text(
            "❌ Settings खोलते समय समस्या हुई। कृपया फिर से Settings दबाएँ।",
            reply_markup=main_menu(),
        )
...
def main():
    ...
    # Main bottom keyboard buttons.
    # Use collect_text for Settings too; it normalizes Telegram's emoji
    # variation selector, which makes the button reliable across clients.
    application.add_handler(
        MessageHandler(
            filters.Regex(
                r"^(?:📝\ufe0f? Quiz|➕ Add Quiz|🔄 ReAttempt|📊 Stats|"
                r"📚 Categories|⚙(?:️)? Settings|Settings|❓ Help)$"
            ),
            collect_text,
        )
    )
